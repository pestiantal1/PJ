...
class SpriteStrip:
    def __init__(self, path, colorkey, number_of_images, width, height, scale=1.0):
        ...
        for i in range(number_of_images):
            img = pygame.Surface((width, height), pygame.SRCALPHA) # felület előkészítése
...
def main():
    ...
    while running:
        ...
        # Billentyűállapot lekérése — controllálható mozgás
        ...
        # Ütközés a képernyő széleivel
        if x < 0:
            x = 0
        if x > SCREEN_WIDTH - frame_w_scaled:
            x = SCREEN_WIDTH - frame_w_scaled

        # Animálás: csak mozgáskor léptetjük a képkockát
        if moving:
            anim_timer += dt
            ...
            # álló pozíció: vissza az első képkockára és reset a counter
            current_frame = animator.image_list[0]
            animator.counter = -1
            anim_timer = 0.0
